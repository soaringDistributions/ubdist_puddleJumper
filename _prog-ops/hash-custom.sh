...
_hash_img-stream() {
    ...
    _wget_githubRelease_join-stdout "soaringDistributions/ubdist_puddleJumper" "$1" "package_image.tar.flx" 2> /dev/null | _get_extract_ubDistBuild-tar --extract ./vm.img --to-stdout | \
...
_hash_rootfs-stream() {
    ...
    _wget_githubRelease_join-stdout "soaringDistributions/ubdist_puddleJumper" "$1" "package_rootfs.tar.flx" 2> /dev/null | lz4 -d -c | \
tee >( wc -c /dev/stdin | cut -f1 -d\ | tr -dc '0-9' > "$safeTmp"/.tmp-currentFileBytes ) | \
tee >( openssl dgst -whirlpool -binary | xxd -p -c 256 > "$safeTmp"/.tmp-whirlpool ) | \
( [[ "$skimfast" != "true" ]] && openssl dgst -sha3-512 -binary | xxd -p -c 256 > "$safeTmp"/.tmp-sha3 )
...
_hash_live-stream() {
    ...
    _wget_githubRelease_join-stdout "soaringDistributions/ubdist_puddleJumper" "$1" "vm-live.iso" 2> /dev/null | \
tee >( wc -c /dev/stdin | cut -f1 -d\ | tr -dc '0-9' > "$safeTmp"/.tmp-currentFileBytes ) | \
tee >( openssl dgst -whirlpool -binary | xxd -p -c 256 > "$safeTmp"/.tmp-whirlpool ) | \
( [[ "$skimfast" != "true" ]] && openssl dgst -sha3-512 -binary | xxd -p -c 256 > "$safeTmp"/.tmp-sha3 )
