...
_get_vmImg_ubDistBuild_sequence() {
	...
	if [[ "$3" == "" ]] # || [[ "$FORCE_AXEL" != "" ]]
	then
		_wget_githubRelease_join-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "package_image.tar.flx" | _get_extract_ubDistBuild-tar xv --overwrite
		currentExitStatus="$?"
	else
		#_wget_githubRelease_join-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "package_image.tar.flx" | _get_extract_ubDistBuild-tar --extract vm.img --to-stdout | _dd of="$3" bs=1M
		_wget_githubRelease_join-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "package_image.tar.flx" | _get_extract_ubDistBuild-tar --extract ./vm.img --to-stdout | sudo -n dd of="$3" bs=1M status=progress
		currentExitStatus="$?"
	fi
	...
	then
		currentHash=$(_wget_githubRelease-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "_hash-ubdist.txt" | head -n 3 | tail -n 1)
		currentHash_bytes=$(_wget_githubRelease-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "_hash-ubdist.txt" | head -n 2 | tail -n 1 | sed 's/^.*count=$(bc <<< '"'"'//' | cut -f1 -d\  )
	fi
...
_get_vmImg_beforeBoot_ubDistBuild_sequence() {
	...
	if [[ "$3" == "" ]] # || [[ "$FORCE_AXEL" != "" ]]
	then
		_wget_githubRelease_join-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "package_image_beforeBoot.tar.flx" | _get_extract_ubDistBuild-tar xv --overwrite
		currentExitStatus="$?"
	else
		#_wget_githubRelease_join-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "package_image_beforeBoot.tar.flx" | _get_extract_ubDistBuild-tar --extract vm.img --to-stdout | _dd of="$3" bs=1M
		_wget_githubRelease_join-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "package_image_beforeBoot.tar.flx" | _get_extract_ubDistBuild-tar --extract ./vm.img --to-stdout | sudo -n dd of="$3" bs=1M status=progress
		currentExitStatus="$?"
	fi
	if [[ "$currentExitStatus" != "0" ]]
	then
	...
	if [[ "$currentHash" == "" ]] || [[ "$currentHash_bytes" == "" ]]
	then
		currentHash=$(_wget_githubRelease-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "_hash-ubdist_beforeBoot.txt" | head -n 3 | tail -n 1)
		currentHash_bytes=$(_wget_githubRelease-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "_hash-ubdist_beforeBoot.txt" | head -n 2 | tail -n 1 | sed 's/^.*count=$(bc <<< '"'"'//' | cut -f1 -d\  )
	fi
	( [[ "$currentHash" == "" ]] || [[ "$currentHash_bytes" == "" ]] ) && _messageFAIL
	export MANDATORY_HASH=
	unset MANDATORY_HASH
...
_get_vmImg_ubDistBuild-live_sequence() {
	...
	then
		...
		_wget_githubRelease_join "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "vm-live.iso"
		currentExitStatus="$?"
	fi
	...
	then
		fallThroughFAIL=PASS
		currentHash_bytes=$(_wget_githubRelease-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "_hash-ubdist.txt" | head -n 14 | tail -n 1 | sed 's/^.*count=$(bc <<< '"'"'//' | cut -f1 -d\  )
		...
		#_wget_githubRelease_join-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "vm-live.iso" | sudo -n wodim -v -sao dev="$3" tsize="$currentHash_bytes" -waiti -
		#-speed=256
		#-dvd-compat
		#-overburn
		#-use-the-force-luke=bufsize:2560m
		#pv -pterbTCB 1G
		#_wget_githubRelease_join-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "vm-live.iso" | sudo -n growisofs -speed=256 -dvd-compat -Z "$3"=/dev/stdin -use-the-force-luke=notray
		
		
		
		
		
		( _wget_githubRelease_join-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "vm-live.iso" | tee >(openssl dgst -whirlpool -binary | xxd -p -c 256 >> "$scriptLocal"/hash-download.txt) ; dd if=/dev/zero bs=2048 count=$(bc <<< '1000000000000 / 2048' ) ) | sudo -n growisofs -speed=3 -dvd-compat -Z "$3"=/dev/stdin -use-the-force-luke=notray -use-the-force-luke=spare:min -use-the-force-luke=bufsize:128m
		currentExitStatus="$?"
		...
		#( _wget_githubRelease_join-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "vm-live.iso" | tee >(openssl dgst -whirlpool -binary | xxd -p -c 256 >> "$scriptLocal"/hash-download.txt) ; dd if=/dev/zero bs=2048 count=$(bc <<< '0 / 2048' ) ) | sudo -n wodim -v -sao fs=128m dev="$3" blank=session tsize="$currentHash_bytes" /dev/stdin
		#currentExitStatus="$?"
		...
		#_wget_githubRelease_join-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "vm-live.iso" | cat > /dev/null
		#currentExitStatus="$?"
	...
	then
		...
		( _wget_githubRelease_join-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "vm-live.iso" | tee >(openssl dgst -whirlpool -binary | xxd -p -c 256 >> "$scriptLocal"/hash-download.txt) ; dd if=/dev/zero bs=2048 count=$(bc <<< '1000000000000 / 2048' ) ) | sudo -n dd of="$3" bs=1M status=progress
		currentExitStatus="$?"
	fi
	...
	if [[ "$currentHash" == "" ]] || [[ "$currentHash_bytes" == "" ]]
	then
		currentHash=$(_wget_githubRelease-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "_hash-ubdist.txt" | head -n 15 | tail -n 1)
		currentHash_bytes=$(_wget_githubRelease-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "_hash-ubdist.txt" | head -n 14 | tail -n 1 | sed 's/^.*count=$(bc <<< '"'"'//' | cut -f1 -d\  )
	fi
	( [[ "$currentHash" == "" ]] || [[ "$currentHash_bytes" == "" ]] ) && _messageFAIL
	export MANDATORY_HASH=
...
_get_vmImg_ubDistBuild-rootfs_sequence() {
	...
	cd "$scriptLocal"
	export MANDATORY_HASH="true"
	_wget_githubRelease_join-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "package_rootfs.tar.flx" | lz4 -d -c > ./package_rootfs.tar
	[[ "$?" != "0" ]] && _messageFAIL
	export MANDATORY_HASH=
	unset MANDATORY_HASH
	...
	unset MANDATORY_HASH
	[[ "$2" != "" ]] && currentHash="$2"
	[[ "$2" == "" ]] && currentHash=$(_wget_githubRelease-stdout "soaringDistributions/ubdist_puddleJumper" "$releaseLabel" "_hash-ubdist.txt" | head -n 9 | tail -n 1)
	export MANDATORY_HASH=
	unset MANDATORY_HASH
